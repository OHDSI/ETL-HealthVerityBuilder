/*Step 2.1: Procedure codes with modifiers in MEDICAL_CLAIMS*/
SELECT T1.DOMAIN_ID,
		T1.PERSON_ID,
		T1.PROVIDER_ID,
		T1.ID, --concat service line number to create a unique id
		T1.CONCEPT_ID,
		T1.SOURCE_VALUE,
		...
		T1.START_TIME,
		T1.DAYS_SUPPLY,
		T1.DOSE_UNIT_CONCEPT_ID,
		T1.DOSE_UNIT_SOURCE_VALUE,
		T1.EFFECTIVE_DRUG_DOSE,
		T1.LOT_NUMBER,
		...
		T1.ANATOMIC_SITE_CONCEPT_ID,
		T1.DISEASE_STATUS_CONCEPT_ID,
		T1.SPECIMEN_SOURCE_ID,
		T1.ANATOMIC_SITE_SOURCE_VALUE,
		T1.DISEASE_STATUS_SOURCE_VALUE,
		CASE
			WHEN T1.SOURCE_CONCEPT_ID IN (
											SELECT TARGET_CONCEPT_ID
											FROM @cdm_schema.SOURCE_TO_SOURCE
											WHERE SOURCE_VOCABULARY_ID = 'HCPCS'
										 )
				THEN
					CASE
						WHEN STAND2.SOURCE_CONCEPT_CLASS_ID = 'HCPCS Modifier'
							THEN TARGET_CONCEPT_ID
						ELSE 0
					END
			WHEN T1.SOURCE_CONCEPT_ID IN (
											SELECT TARGET_CONCEPT_ID
				...
				THEN
					CASE
						WHEN STAND2.SOURCE_CONCEPT_CLASS_ID = 'CPT4 Modifier'
							THEN TARGET_CONCEPT_ID
						ELSE 0
					END
			ELSE 0
...
(
	SELECT
	 ...
	 MC.CLAIM_ID+cast(MC.RECORD_ID as varchar(32))+DATA_FEED AS ID, --concat service line number to create a unique id
		...
		NULL AS LOT_NUMBER,
		PROCEDURE_MODIFIER_1 AS MODIFIER_SOURCE_VALUE,
		...
		CASE
			WHEN SOURCE1.SOURCE_VOCABULARY_ID IS NOT NULL
				THEN SOURCE1.SOURCE_VOCABULARY_ID
			ELSE NULL
	...
	FROM @source_schema.MEDICAL_CLAIMS MC
		...
		JOIN @cdm_schema.PERSON P								/*~~~Check this join - we should be able to do an inner join here but leaving as left for now, also take out 'sample'~~~*/
			...
			AND SUBSTRING(HVID,1,@n_substring) = '@HVID_CHUNK'
		LEFT JOIN @cdm_schema.PROVIDER PR
			ON MC.PROV_RENDERING_NPI = PR.PROVIDER_SOURCE_VALUE
		--JOIN @cdm_schema.FINAL_VISIT_IDS V
			--ON V.STEM_ID = MC.DATA_FEED+'-'+MC.CLAIM_ID+'-'+cast(MC.RECORD_ID as varchar(32)) /*~~~Check this join with other lefts~~~*/
	WHERE PROCEDURE_CODE IS NOT NULL AND PROCEDURE_MODIFIER_1 IS NOT NULL
) T1

LEFT JOIN @cdm_schema.SOURCE_TO_STANDARD AS STAND2
	ON MODIFIER_SOURCE_VALUE = STAND2.SOURCE_CODE_NO_DECIMAL
	AND STAND2.VOCAB_LABEL = 'PROCEDURE_MODIFIERS'
  AND STAND2.SOURCE_VOCABULARY_ID = T1.SOURCE_VOCABULARY_ID
